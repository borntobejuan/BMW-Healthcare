...
Punto de entrada principal. Ejecuta uno de los modos:
  ...
  python main.py live           → Pipeline tiempo real → InfluxDB → Grafana
...
Antes de usar:
  ...
  3. Ajusta CAN_PORT en .env (ej. COM3)
...
load_dotenv()   # Carga .env antes de importar módulos que leen os.getenv
...
def cmd_live(args):
    from telemetry.pipeline import LivePipeline
    port = args.port or detect_kdcan_port() or SERIAL_PORT
    pipeline = LivePipeline(port=port)
...
def main():
    ...
    # live
    p_live = sub.add_parser("live", help="Pipeline tiempo real → InfluxDB → Grafana")
